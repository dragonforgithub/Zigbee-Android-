...
public class StartActivity extends Activity implements OnClickListener{
	...
	private void refreshDeviceList() {
		...
		if(mEntries.size()>0){
			...
			msgTv.setText(getResources().getString(R.string.Zigbee_connected)+","+title);
		}else{
			//判断wifi已连接的条件
			if(wifi == State.CONNECTED||wifi==State.CONNECTING){
				msgTv.setText(getResources().getString(R.string.WiFi_connected));
			}else{
				msgTv.setText(getResources().getString(R.string.ZigbeeOrWifi_not_connect));
			}
